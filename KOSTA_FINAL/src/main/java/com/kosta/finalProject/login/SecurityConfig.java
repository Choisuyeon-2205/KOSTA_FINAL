...
public class SecurityConfig extends WebSecurityConfigurerAdapter  {
	...
	public void configure(WebSecurity web) throws Exception {
		...
		web.ignoring().antMatchers("/oauth2/**","/css/**", "/js/**", "/image/**", "/lib/**");
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests() // HttpServletRequest에 따라 접근(access)을 제한
			...
			.antMatchers("/main/**","/fragments/**","/center/centerlist","/login/**","/exerciseinfoboard/boardlist","/dietdiaryboard/boardlist","/healthboard/boardlist","/sharingboard/boardlist","/naversearch/**", "/upload/**", "/body/**").permitAll() //   누구나 접근 허용
